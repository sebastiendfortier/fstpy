...
def get_arguments():
    ...
    parser.add_argument(
        "--suffix", type=str, default="", help="Suffix to add to the package name (ex: beta or operation)."
    )
    parser.add_argument("--install", action="store_true", help="Do a local install.")
    parser.add_argument(
        "--temp", type=str, help="Temporary path to install the package if the --install option is used."
    )
...
def install(pkg_name, dir_livraison, path, name, version):
    ...
    process = subprocess.run(
        ["ssm", "install", "-f", dir_livraison + "/" + pkg_name + ".ssm", "-d", os.path.join(temp_path, "master")]
    )

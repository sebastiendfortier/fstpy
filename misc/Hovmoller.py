...
def create_plot(lons, vtimes, avg_data):
    ...
    ax1.set_yticklabels(["40\N{DEGREE SIGN}N", "60\N{DEGREE SIGN}N"])
    ...
    cs = ax2.contour(lons, vtimes, mpcalc.smooth_n_point(avg_data, 9, 2), clevs, colors="k", linewidths=1)
...
def get_vwnd_subset(times, vwnd):
    ...
    data = vwnd.sel(time=time_slice, lat=lat_slice, lon=lon_slice)
    return data
...
def get_latitude_data_array(lat_lon_df):
    ...
    lat = xr.DataArray(lati, dims=["lat"], coords=dict(lat=lati), name="lat", attrs=lat_arrtibs)
    return lat
...
def get_longitude_data_array(lat_lon_df):
    ...
    lon = xr.DataArray(loni, dims=["lon"], coords=dict(lon=loni), name="lon", attrs=lon_arrtibs)
    return lon
...
def get_time_data_array(vv):
    ...
    time = xr.DataArray(times, dims=["time"], coords=dict(time=times), name="time", attrs=time_arrtibs)
    return time, times


def get_dataframe_sorted_by_date():
    df = fstpy.StandardFileReader(
        "/home/spst900/spooki/spooki_dir/pluginsRelatedStuff/testsFiles/fstpy_hovmoller_VV_250hPa_RU_0.3d",

...
import numpy as np
import pandas as pd
from typing import Tuple, Final, List, Union, Dict, Any
from .rmn_interface import RmnInterface, VGridInterface
...
vctype_dict = {
    ...
    "HYBRID_5100": VerticalCoordType.HYBRID_5100,
...
class VerticalCoord(ABC):
    ...
    def __init__(
        self, file_df: pd.DataFrame, meta_df: pd.DataFrame, df: pd.DataFrame, vcode=0, kind=0, version=0
    ) -> None:
        self.vcode = vcode
        self.kind = kind
    ...
    def get_levels_and_ips_df(self):
        ...
        def getLevel(ip):
            (level, _) = RmnInterface.convert_ip(RmnInterface.CONVIP_DECODE, int(ip))
            return level
    ...
    def create_pxsa_container(self) -> pd.DataFrame:
        ...
        :rtype: pd.DataFrame
        """
        res_df = self.create_result_container(
            "PXSA", "PRESSR", "hectoPascal", "Pressure of the model standard atmosphere"
        )
        return res_df
...
class VerticalCoordHybrid(VerticalCoord):
    ...
    def __init__(
        self, file_df: pd.DataFrame, meta_df: pd.DataFrame, df: pd.DataFrame, vcode: int, kind: int, version: int
    ) -> None:
    ...
    def hybrid_pressure_with_toctoc(self) -> List[da.Array]:
        ...
        s = np.log(p0 * 100.0 / self.pref)
        pres = [
            (np.exp(self.vcoord_table.at[idx, "A"] + self.vcoord_table.at[idx, "B"] * s) / 100.0).astype(np.float32)
    ...
    def hybrid_pressure_with_toctoc_sa(self) -> List[da.Array]:
        ...
        self.get_vcoord_table()
        p0 = self.p0_df.iloc[0].d
        self.vcoord_table["std_atm_value"] = (
            np.exp(self.vcoord_table["A"] + self.vcoord_table["B"] * math.log(STANDARD_ATMOSPHERE * 100.0 / self.pref))
            / 100.0
        )
        pres = [
            da.full(p0.shape, self.vcoord_table.at[idx, "std_atm_value"], dtype=np.float32, order="F")
            for idx in self.vcoord_table.index
        ]
        return pres
...
class VerticalCoord5001(VerticalCoord):
    ...
    def pressure(self):
        ...
        pres = [
            (vcoord_table.at[idx, "A"] + vcoord_table.at[idx, "B"] * p0).astype(np.float32)
    ...
    def pressure_standard_atmosphere(self):
        ...
        evalTerm1 = np.where(term4 < 0, 0.0, term4)
...
def get_vertical_coordinate_type(
    kind: int, toctoc: bool, p0: bool, pt: bool, hy: bool, vcode: list, check_p0: bool = True
) -> str:
    result = "UNKNOWN"
...
def set_vertical_coordinate_type(df: pd.DataFrame) -> pd.DataFrame:
    ...
    from .dataframe_utils import convert_cols_to_boolean_dtype
    from . import VCTYPES
        ...
        for (grid, ip1_kind), group_df in groups:
            ...
            if vctype == "METER_GROUND_LEVEL" and 5005 in vcode:
                ...
                for nomvar, nomvar_df in nomvar_groups:
                    group_df.loc[(group_df.nomvar == nomvar) & (group_df.level.isin([1.5, 10])), "vctype"] = (
                        vctype_dict["HYBRID_5005"]
                    )
    ...
    # vctype in columns
        ...
        if df.loc[df.vctype.isna()].empty:
            ...
            # Necessaire pour s'assurer que les colonnes sont bien de type bool (warning)
            bool_cols = ["surface", "ascending", "follow_topography"]
            df = convert_cols_to_boolean_dtype(df, bool_cols)

            missing_vctype_df = df.loc[df.vctype.isna()]
            correct_vctype_df = df.loc[~df.vctype.isna()]
            meta_df = df.loc[df.nomvar.isin(["^^", ">>", "^>", "!!", "!!SF", "HY", "P0", "PT"])]
            missing_vctype_with_meta_df = safe_concatenate([meta_df, missing_vctype_df])
            ...
            res_df = safe_concatenate([new_df, correct_vctype_df])
...
def get_df_from_vgrid(vgrid_descriptor: "VGridInterface.VGridDescriptor", ip1: int, ip2: int) -> pd.DataFrame:
    """Creates a dataframe from a vgrid descriptor

    :param vgrid_descriptor: vgrid descriptor
    :type vgrid_descriptor: VGridInterface.VGridDescriptor
    :param ip1: ip1 value
    :type ip1: int
    :param ip2: ip2 value
    :type ip2: int
    :return: dataframe containing the vgrid descriptor information
    :rtype: pd.DataFrame
    """
...
def vertical_coord_to_dict(vgrid_descriptor: "VGridInterface.VGridDescriptor") -> dict:
    """Creates a dictionnary from the vgrid_descriptor
    ...
    """
    vcoord = {}
    vcoord["KIND"] = VGridInterface.get_vgrid_param(vgrid_descriptor, "KIND")
    vcoord["VERSION"] = VGridInterface.get_vgrid_param(vgrid_descriptor, "VERSION")
    vcoord["VTBL"] = np.asfortranarray(np.squeeze(VGridInterface.get_vgrid_param(vgrid_descriptor, "VTBL")))
    return vcoord
